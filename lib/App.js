...
const { blank, filled, wait } = require('./Utils');
...
class App extends OAuth2App {
  ...
  */
  ...
  async syncData(client) {
    let [devices, notifications] = await Promise.all([
  ...
  */
  ...
  registerFlowCards() {
    ...
    this.homey.flow.getDeviceTriggerCard('battery_level_changed').registerRunListener(async ({
      device,
      battery_level,
    }) => {
