...
class Device extends OAuth2Device {
  ...
  */
  ...
  // Synchronize device
  async syncDevice() {
    let device = this.homey.app.devices[this._id] || {};
    if (blank(device)) return;

    // Update capabilities
    for (const [key, value] of Object.entries(device.capabilityValues)) {
      this.setCapabilityValue(key, value).catch(this.error);
    }

    // Update settings
    this.setSettings(device.settings).catch(this.error);
  ...
  async syncNotifications() {
    ...
    let messages = this.homey.app.notifications[this._id] || [];
