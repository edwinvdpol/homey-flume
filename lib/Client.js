...
class Client extends OAuth2Client {
  ...
  */
  ...
  async _get(path, log = true) {
    ...
    const result = await this.get({ path, query: '', headers: {} });
  ...
  async _patch(path, json = null) {
    ...
    const result = await this.patch({ path, query: '', json, body: null, headers: {} });
  ...
  async _post(path, json = null) {
    ...
    const result = await this.post({ path, query: '', json, body: null, headers: {} });
  ...
  */
  ...
  async onGetTokenByCredentials({ username, password }) {
    ...
    const response = await fetch(this._tokenUrl, { body, method: 'POST' });
